package com.eibrahim67.gympro.ui.showExercise.view.fragments
...
import com.eibrahim67.gympro.ui.showExercise.view.adapters.AdapterRVExercisesResults
import com.eibrahim67.gympro.ui.showExercise.view.bottomSheets.BottomSheetAddNewSet
                    ...
                    exercise.data?.let { data ->
                        ...
                        Glide.with(requireContext()).load(data.imageUrl).apply(
                            RequestOptions().diskCacheStrategy(DiskCacheStrategy.ALL)
                                .placeholder(R.color.gray_v1).error(R.drawable.error_ic)
                        ).into(binding.exerciseImage)

                        binding.exerciseTitle.text = data.name
                        binding.exerciseHint.text = data.exerciseHint

                        Toast.makeText(
                            requireContext(), data.imageUrl, Toast.LENGTH_SHORT
                        ).show()
                    } ?: run {
                        Toast.makeText(
