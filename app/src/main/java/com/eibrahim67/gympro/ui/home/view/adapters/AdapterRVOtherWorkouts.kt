...
class AdapterRVOtherWorkouts(
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        differ.currentList[position].imageUrl.let { url ->
            Glide.with(context).load(url).centerCrop().into(holder.imageOthersWorkout)
        }
    ...
    private val differ: AsyncListDiffer<Workout> = AsyncListDiffer(this, DIFF_CALLBACK)
    ...
    companion object {
        val DIFF_CALLBACK = object : DiffUtil.ItemCallback<Workout>() {
            override fun areItemsTheSame(
                oldItem: Workout, newItem: Workout
            ): Boolean = oldItem === newItem // this is data class
    ...
    class CategoryViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val avgTimeOthersWorkout: TextView = itemView.findViewById(R.id.itemAvgTimeOthersWorkout)
